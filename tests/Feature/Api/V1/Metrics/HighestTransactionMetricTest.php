...
class HighestTransactionMetricTest extends MetricsTestCase
{
    ...
    public function test_returns_highest_transaction_by_account(): void
    {
        ...
        $response = $this->getJson("/api/v1/metrics/highest-transaction?from={$from}&to={$to}");

        $response->assertOk();
        $items = $response->json('data.items');
        $this->assertIsArray($items);
        $foodAccount = collect($items)->firstWhere('label', 'Food');
        $this->assertEquals(500, $foodAccount['value']);
    }
